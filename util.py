...
def write_tractory(file_path, coords, time_stamp, skip_list=[]):
    ...
    if len(coords) > 3:
        data = {
            ...
            'yaw[rad]': [coord[3] for coord in coords],
            'state[state]': [coord[4] for coord in coords],
...
def opt_schedule(flss, Lambda, opening, fly_through_speed, Salt=0):
    interArrivalTime = (1 / Lambda) + Salt
    schedule = []
    minDepartTime = 0
    ...
    for i, fls in enumerate(flss):
        ...
        _, _, travel_time = fls.linear_movement_OPT(dist, fly_through_speed, float('inf'), error=0)
...
def opt_schedule_dict(flss, Lambda, opening, fly_through_speed, salt):
    schedule = opt_schedule(flss, Lambda, opening, fly_through_speed, salt)
    s_dict = {}
